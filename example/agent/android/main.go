...
func main() {
	...
	androidAgent := agent.NewMobileAgent("mobile use agent", "118.31.173.101:100",
		...
		agent.WithStorage(storage.NewJSONStorage("tmp/leads_agent")),
	)
...
1. 根据给你的截图确认当前是否在[抖音-团购-丽人/美发]页然后在执行 2，如果不在请按序操作以下不走
	- 打开抖音；
	- 点击团购tab；
	- 点击城市按钮（**页面中可能存在多个城市按钮，注意区分你要点击的城市按钮通常跟搜索在一块**），切换城市为北京；
	- 进入"丽人/美发"频道
2. 筛选三里屯商圈的商家
3. 提取20条不同商家信息（名称、地址、主营类目、门店性质、是否开设直播、团购数量、评分、评论数、营业执照信息） ，信息提取步骤如下：
	- 点击商家卡片，进入商家详情页，提取名称、地址、主营类目、门店性质、是否开设直播、团购数量、评分、评论数等主要信息
	- 点击商家详情页右上角的"..."按钮
	- 点击"商家资质"按钮
	- 查看并截图商家的营业执照信息
	- 返回商家列表页
	- 点击下一个商家，重复上述步骤，直到提取20条不同商家信息
4. 批量保存收集到的线索信息
`
