...
func (rp *react) WithTools(tools ...*llm.Tool) *react {
	instruction := rp.Instruction
	toolString := ""
	for i := 0; i < len(tools); i++ {
		toolString += strconv.Itoa(i+1) + ". "
		toolString += tools[i].Function.Name + ":" + tools[i].Function.Description
		rawArgs, _ := json.Marshal(tools[i].Function.Parameters)
